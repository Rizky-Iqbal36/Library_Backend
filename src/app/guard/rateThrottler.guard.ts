...
export class RateThrottlerGuard extends ThrottlerGuard {
  async handleRequest(context: ExecutionContext, limit: number, ttl: number): Promise<boolean> {
    ...
    //   if (banRecordCount >= 3) {
    //     const remainingTime = await promisify(redis.client.client.ttl).apply(redis.client.client, [banKey])

    //     throw new TooManyRequestException(httpFlags.REQUEST_BANNED, { localeMessage: { key: 'MESSAGE_REQUEST_BANNED', vars: { time: getLocalizedTime(remainingTime, lang) } } })
    //   }
    //   await redis.client.set(banKey, (banRecordCount + 1).toString(), { ttlInSec: 3600 })
    // }
